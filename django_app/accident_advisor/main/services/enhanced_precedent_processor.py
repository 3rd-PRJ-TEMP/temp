...
class EnhancedPrecedentProcessor:
    ...
    def extract_court_name(self, query: str) -> Optional[str]:
        ...
        for normalized_court, variations in self.COURT_NORMALIZATION.items():
        ...
        try:
            ...
            if case_number:
                ...
                exact_matches = self.precedent_db.similarity_search(
                    query=case_number,
                    filter={self.METADATA_KEY['CASE_ID']: case_number},
                    k=5
                )
                ...
                for doc in partial_matches:
                    doc_case_id = doc.metadata.get(self.METADATA_KEY['CASE_ID'], '').lower()
            ...
            if court_name:
                court_matches = self.precedent_db.similarity_search(
                    query=court_name,
                    filter={self.METADATA_KEY['COURT']: court_name},
                    k=5
                )
    ...
    def _generate_exact_match_response(self, user_input: str, docs: List[Document]) -> str:
        ...
        try:
            ...
            context = "\n\n".join([
                f"판례 {i+1}:\n"
                f"법원: {doc.metadata.get(self.METADATA_KEY['COURT'], 'N/A')}\n"
                f"사건번호: {doc.metadata.get(self.METADATA_KEY['CASE_ID'], 'N/A')}\n"
...
**1️⃣ {docs[0].metadata.get(self.METADATA_KEY['CASE_ID'], 'N/A')}**
• **법원**: {docs[0].metadata.get(self.METADATA_KEY['COURT'], 'N/A')}
• **사고 개요**: [사고 상황을 1-2줄로 간단히]
• **주요 판단**: [핵심 법적 판단 1-2줄] 
• **과실비율**: [명시된 경우만 표시]

{f'''**2️⃣ {docs[1].metadata.get(self.METADATA_KEY['CASE_ID'], 'N/A')}**
• **법원**: {docs[1].metadata.get(self.METADATA_KEY['COURT'], 'N/A')}
• **사고 개요**: [사고 상황을 1-2줄로 간단히]
• **주요 판단**: [핵심 법적 판단 1-2줄]
...
**📞 다른 방식으로 질문해주시면 더 정확한 도움을 드릴 수 있습니다!**"""
    ...
    def _process_general_precedent_search(self, user_input: str) -> str:
        ...
        try:
            ...
            metadata_field_info = [
                AttributeInfo(
                    name=self.METADATA_KEY['COURT'],
                    description="판례의 법원명 (예: 대법원, 서울고등법원, 지방법원 등)",
                    type="string"
                ),
                AttributeInfo(
                    name=self.METADATA_KEY['CASE_ID'],
